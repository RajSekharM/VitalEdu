<?php
	...
	if(isset($_SESSION['adminSession'])){
		...
		$required = array('tutor_id', 'tutor_name', 'tutor_address', 'tutor_salary', 'tutor_email', 'tutor_mobile');
	...
	if ($error) {
		...
		$tutor_mobile = $_POST['tutor_mobile'];
				
		$sql = "INSERT INTO tutor (`tutor_id`, `tutor_name`, `tutor_address`, `tutor_salary`, `tutor_email`, `tutor_mobile`) VALUES ('$tutor_id', '$tutor_name', '$tutor_address', '$tutor_salary', '$tutor_email', '$tutor_mobile')";

		if (mysqli_query($conn, $sql)) {			
			$done = 'true';
		header( "refresh:5;url=tutorlist.php" );	
		} else {
			$done = 'false';
		} 
	}?>
			...
			<div class="col-md-10 content">
				<div class="panel panel-default">
					<div class="panel-heading text-center">
						<strong><big>ADD NEW TUTOR</big></strong>
					</div>
					<div class="panel-body text-center" style = "padding: 10%">
					
					<?php 
						if($done == 'true'){ ?>
							<big>TUTOR ADDED SUCCESSFULLY</big>
					<?php	}
						else if($done == 'false'){ ?>
							<big>ERROR ADDING TUTOR</big>
