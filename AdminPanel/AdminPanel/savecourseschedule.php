<?php
	...
	$course_locationid = $_POST['course_location'];
	$course_tutorid = $_POST['course_tutor'];
	$date = $_POST['course_date'];
	$course_start = $_POST['course_start'];
	$course_end = $_POST['course_end'];
	$course_seats = $_POST['course_seats'];	  	 
	 	
	$week = date('l', strtotime($date));
	
	$done = "false";
	
$sql = "INSERT INTO course_timetable (`course_id`, `location_id`, `tutor_id`, `month_id`, `day_id`, `date_id`, `starttime_id`, `endtime_id`, `avail_seats`) VALUES ('$course_id', '$course_locationid', '$course_tutorid', EXTRACT(MONTH FROM '$date'), '$weekday[$week]', EXTRACT(DAY FROM '$date'), '$course_start', '$course_end', '$course_seats')";

if (mysqli_query($conn, $sql)) {			
		$done = 'true';
		header( "refresh:5;url=courseschedule.php" );	
	} else {
		$done = 'false';
	} ?>
			...
			<div class="col-md-10 content">
				<div class="panel panel-default">
					<div class="panel-heading text-center">
						<strong><big>ADD COURSE SCHEDULE</big></strong>
					</div>
					<div class="panel-body text-center" style = "padding: 10%">
					
					<?php 
						if($done == 'true'){ ?>
							<big>COURSE SCHEDULE ADDED SUCCESSFULLY</big>
					<?php	}
						else if($done == 'false'){ ?>
							<big>ERROR ADDING COURSE SCHEDULE</big>
